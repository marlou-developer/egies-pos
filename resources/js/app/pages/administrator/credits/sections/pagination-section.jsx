...
export default function PaginationSection() {
    ...
    const pageNumbers = generatePageNumbers(carts?.last_page || 1, current);
        ...
        <div className="w-full mt-4">
            {carts?.last_page > 1 && (
                <div className="flex justify-between items-center flex-wrap gap-y-2">
                    <div>Total: {carts?.total} entries</div>
                    <div className="flex space-x-1 items-center">
                        ...
                        <Link
                            href={createLink(current + 1)}
                            className={`px-3 py-1 border rounded ${
                                current === carts.data.last_page
                                    ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                                    : "bg-white text-pink-500 border-pink-500"
                            }`}
                            aria-disabled={current === carts.data.last_page}
                        >
                            Next
                        </Link>
                    </div>
