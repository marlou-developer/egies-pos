...
export default function FilterCreditsSection() {
    // Get current search parameter from URL
    const urlParams = new URLSearchParams(window.location.search);
    const currentSearch = urlParams.get("search") || "all";

    const [creditStatus, setCreditStatus] = useState(currentSearch);

    function search_products() {
        // If "All Credits" is selected or no value, just visit the base URL
        if (!creditStatus || creditStatus === "all") {
            router.visit(window.location.pathname); // Refresh without any filter
        } else {
            router.visit(`?search=${creditStatus}`);
        }
    }
