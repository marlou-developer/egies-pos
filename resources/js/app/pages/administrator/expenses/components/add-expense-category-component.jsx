...
import DeleteExpenseCategorySection from "../sections/delete-expense-category-section";
import EditExpenseCategorySection from "../sections/edit-expense-category-section";
import { create_expense_category_thunk, get_expense_category_thunk } from "@/app/redux/expense-category-thunk";

export default function AddExpenseCategoryComponent({ open, setOpenCategory }) {
    ...
    const createCategory = async (e) => {
        ...
        try {
            await store.dispatch(
                create_expense_category_thunk({
                    ...form,
                })
            );
            message.success("Successfully added!");
            await store.dispatch(get_expense_category_thunk());
            setOpenCategory(false);
        } catch (error) {
            message.error("Failed to add expense category. Please try again.");
        } finally {
            setLoading(false);
        }
    };

    console.log("Expense Categories:", expense_categories);
                    ...
                    <div className="h-0 flex-1 overflow-y-auto">
                        <div className="bg-pink-200 px-4 py-6 sm:px-6">
                            ...
                            <p className="mt-1 text-sm text-gray-600">
                                Fill in the information below to create a new expense category.
                            </p>
                        </div>
                                        ...
                                        {expense_categories?.map((category, idx) => (
                                            ...
                                            >
                                                ...
                                                {(user?.user_type === "Admin" || user?.user_type === "Super Admin") && (
                                                    <EditExpenseCategorySection
                                                        data={category}
                                                    />
                                                )}
                                                {(user?.user_type === "Admin" || user?.user_type === "Super Admin") && (
                                                    <DeleteExpenseCategorySection
                                                        data={category}
                                                    />
                                                )}
                                            </span>
