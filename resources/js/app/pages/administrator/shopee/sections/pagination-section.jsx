...
export default function PaginationSection() {
    ...
    const { shopees } = useSelector((store) => store.carts);
    ...
    const pageNumbers = generatePageNumbers(shopees?.last_page || 1, current);

    const createLink = (pageNum) => `?page=${pageNum}&search=${search ?? ""}`;

    console.log("shopeesshopees", shopees);
    return (
        <div className="w-full mt-4">
            {shopees?.last_page > 1 && (
                <div className="flex justify-between items-center flex-wrap gap-y-2">
                    <div>Total: {shopees?.total} entries</div>
                    <div className="flex space-x-1 items-center">
                        ...
                        {pageNumbers.map((pageNum, index) => {
                            if (
                                pageNum === "left-ellipsis" ||
                                pageNum === "right-ellipsis"
                            ) {
                                return (
                        ...
                        <Link
                            href={createLink(current + 1)}
                            className={`px-3 py-1 border rounded ${
                                current === shopees.data.last_page
                                    ? "bg-gray-200 text-gray-500 cursor-not-allowed"
                                    : "bg-white text-pink-500 border-pink-500"
                            }`}
                            aria-disabled={current === shopees.data.last_page}
                        >
                            Next
                        </Link>
                    </div>
